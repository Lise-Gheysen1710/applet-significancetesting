...
MAX_SIMS = 1000000;
TEST_TYPES = {"right": "One-tailed right", "left": "One-tailed left", "twosided": "Two-tailed"}

ui.page_opts(title="Testing of hypotheses with simulations - applet")

with ui.sidebar(width=400):
    ui.markdown("<strong>Settings simulation</strong>");
    #USER INPUT ELEMENTS
    ui.input_numeric("sampleSize", "Sample size n",value=500,min=100,max=10000);
    ui.input_slider("observedProportion", "Observed sample proportion p\u0302", value=0.32, min=0, max=1, step=0.005);
    ui.input_slider("nullHypothesis", "Null hypothesis p\u2080", value=0.3, min=0, max=1, step=0.005);
    ui.input_numeric("simulationSize", "Number of simulations if the null hypothesis is valid",value=10000,min=1,max=MAX_SIMS,step=1);
    ui.input_radio_buttons("testType", "Type of test:", TEST_TYPES, selected="right", inline=False)
    ui.markdown("<strong>Visualisation settings</strong>");
    ui.input_checkbox("showThreshold", "Show threshold on histograms",value=False);
    ui.input_switch("useProportions", "Use proportions",False);

ui.markdown("<strong>Results simulation</strong>");
...
def showHistogram():
  ...
  ax1.set_xlabel('Number of successes in one sample (X)');
  ...
  ax2.set_xlabel('Number of successes in one sample (X)');
  ...
  plt.suptitle('Histograms of sampling distribution');
  return fig

with ui.card():
  ui.card_header("Threshold value (n)")
  @render.ui
...
with ui.card():
  ui.card_header("Emperical p-value");
  @render.ui
...
with ui.accordion(id="acc", open=False,):  
  with ui.accordion_panel("Table with simulation data"):  
    @render.data_frame  
    def showTable():
      sampleSize = input.sampleSize();
      if sampleSize < 100:
      ...
      df = pd.DataFrame(my_array, columns=['Simulation number','# successes','Proportion','Threshold value crossed?'])
